...
(function(window, document, angular) {
    ...
    angular.module('ngFitText', [])
        ...
        .directive('fittext', [
            ...
            'fitTextConfig',
            'fitTextService',
            function($timeout, config, fitTextConfig, fitTextService) {
                return {
                    restrict: 'A',
                    scope: true,
                    link: function(scope, element, attrs) {
                        ...
                        var parent          = element.parent()
                            ...
                            , compressor      = isNaN(parseInt(attrs.fittextCompress)) ? config.compressor : attrs.fittextCompress
                        ...
                        var resizer = this.resize = function () {
                            if(resizePromise)
        ...
        .service('fitTextService', [function () {
            var self = this;
            var fitTexts = {};

            self.register = function (id, link) {
                fitTexts[id] = link;
            };
            ...
            self.forceRecalculate = function (id) {
                id.split(' ').forEach(function (i) {
                    var link = fitTexts[i];
                    if (link && typeof link.resize === 'function') {
                        link.resize();
                    }
                });
            };
